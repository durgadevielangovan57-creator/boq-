...
export default function BomApprovals() {
    ...
    const visibleApprovals = statusFilter === "all"
        ...
        : dateFilteredApprovals.filter(a => {
            if (statusFilter === "pending") return a.status === "pending_approval" || a.status === "submitted";
    ...
    const sortByDate = (list: BOMApproval[]) => {
    ...
    const sortedPending = sortByDate(visibleApprovals.filter(a => a.status !== 'edit_requested'));
    const sortedEditRequests = sortByDate(visibleApprovals.filter(a => a.status === 'edit_requested'));
    ...
    const pendingApprovals = dateFilter ? sortedPending : sortedPending.slice(0, 10);
