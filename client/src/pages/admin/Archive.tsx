...
import { RefreshCw, Trash2, Archive as ArchiveIcon, Search } from "lucide-react";
...
export default function Archive() {
  ...
  const handleBulkTrash = async () => {
    const ids = Array.from(selected);
    await Promise.all(ids.map(id => handleTrash(id)));
  };
  ...
  return (
    <Layout>
      <div className="container mx-auto px-4 py-8 max-w-7xl">
        ...
        <Card>
          <CardHeader className="border-b bg-gray-50/50 space-y-3">
            ...
            {someSelected && (
              <div className="flex items-center gap-3 py-2 px-3 bg-blue-50 border border-blue-200 rounded-md">
                ...
                <div className="flex gap-2 ml-auto">
                  ...
                  >
                    <RefreshCw className="h-4 w-4 mr-1" /> Restore Selected
                  </Button>
                  <Button
                    variant="outline"
                    size="sm"
                  ...
                  >
                    <Trash2 className="h-4 w-4 mr-1" /> Trash Selected
                  </Button>
                </div>
              </div>
            )}
                ...
                <table className="w-full text-sm text-left">
                  ...
                  <tbody>
                    {filteredItems.map((item) => (
                      <tr
                        key={item.id}
                        className={`border-b hover:bg-gray-50 ${selected.has(item.id) ? "bg-blue-50/50" : "bg-white"
                          }`}
                      >
