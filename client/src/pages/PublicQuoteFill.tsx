...
// Fully public route (no login, no sidebar) - mobile friendly. Meant to feel like a Google Form.
export default function PublicQuoteFill() {
    ...
    const load = () => {
        setLoading(true);
        apiFetch(`/api/fb/public/quotes/${token}`)
            .then((r) => {
            ...
            .then((d) => {
                ...
                setItems(d.items || []);
                setVendor(d.vendor);
                setSubmitted(d.recipient?.status === "Submitted");
    ...
    const submit = async () => {
        ...
        try {
            ...
            const res = await apiFetch(`/api/fb/public/quotes/${token}/respond`, {
                method: "POST",
            ...
            <div className="min-h-screen flex items-center justify-center bg-slate-50 p-6">
                <Card className="max-w-md w-full text-center">
                    <CardContent className="pt-10 pb-10 space-y-3">
                        ...
                        <p className="text-sm text-muted-foreground">Thank you{vendor?.name ? `, ${vendor.name}` : ""}. Your rates have been recorded.</p>
                    </CardContent>
                </Card>
            </div>
        );
            ...
            <div className="max-w-2xl mx-auto space-y-4">
                <Card>
                    <CardHeader className="text-center border-b pb-4">
                        ...
                        {vendor && <Badge variant="secondary" className="mx-auto mt-2 w-fit">For {vendor.name || vendor.username}</Badge>}
                    </CardHeader>
                </Card>
