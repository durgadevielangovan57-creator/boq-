...
// ── Generate PO: standalone "Approvals" panel (NEW, isolated) ──────────────
// Mirrors the exact content of /po-approvals (POApprovals.tsx) — same data
// sources (purchase-orders + po-requests, both pending_approval), same
// status filter (Pending/Approved/Rejected), same "View" navigation, and
// the same Approve/Reject dialog with a comment box — just without that
// page's own <Layout> wrapper, so it can be dropped into a dialog. This
// file is additive only; it doesn't modify POApprovals.tsx, PurchaseOrders,
// or any other existing page.
...
type PoApprovalsPanelProps = {
    onActioned?: () => void;
    onNavigate?: () => void; // called right before we route away (e.g. to close a parent dialog)
    returnPath?: string; // where "Back to List" on the detail page should return to, instead of /po-approvals
};

export default function PoApprovalsPanel({ onActioned, onNavigate, returnPath }: PoApprovalsPanelProps) {
                                            ...
                                            <TableCell className="text-right">
                                                <div className="flex justify-end gap-2">
                                                    <Button variant="outline" size="sm" onClick={() => {
                                                        onNavigate?.();
                                                        const base = item.type === 'Annexure' ? `/purchase-orders/${item.id}?mode=approval` : `/po-requests/${item.id}?mode=approval`;
                                                        setLocation(returnPath ? `${base}&returnTo=${encodeURIComponent(returnPath)}` : base);
                                                    }}>
